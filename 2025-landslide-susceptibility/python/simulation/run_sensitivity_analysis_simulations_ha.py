...
#
# Script for running the simulations for the general sensitivity 
# analysis using the plots with observed landslides.
#
...
def process_setup(plot_path, cfg, num_processes = 1):
    ...
    param_in_path = os.path.join(result_base_path,"sensitivity_param_values.txt")
    ...
    # Only process plots with observed slides
    if plot_sensitivity:
        ...
        for i in range(len(param_values)):
            [c_mean,fc_mean, ks_mean, por_mean, r_mean,rainfall] = param_values[i]

            output_name = cfg["output_name_template"].format(i)
            rec = {
                "c_mean": c_mean,"fc_mean": fc_mean, "ks_mean": ks_mean, "por_mean": por_mean, "r_mean": r_mean, "rainfall": rainfall,
                "step":i,
...
def process_simulation(sim_record, cfg):
    ...
    with open(comb_path, "w") as file:
        ...
        writer.writerow(["i","c","fc","ks", "por", "r","rainfall","min","max","mean","p5","p10","q1","median","q2","p90","p95","cells","cells_buffered","ls_failed","ls_failed_all"] )    
        writer.writerow([step,c_mean,fc_mean, ks_mean, por_mean, r_mean,rainfall,pf_min, pf_max, pf_mean, pf_5,p_10,pf_q1, pf_median, pf_q2,pf_90,pf_95,perimeter_cell_count, perimeter_buffered_cell_count, ls_failed_count, ls_failed_all_count])  
...
def collect_results(plot_path,cfg):
    ...
    with open( summary_out_path, "w", newline="") as comb_file:
        ...
        # Write header
        comb_writer.writerow(["plot_fid","i","c","fc","ks", "por", "r","rainfall","min","max","mean","p5","p10","q1","median","q2","p90","p95","cells","cells_buffered","ls_failed","ls_failed_all"] )
        ...
        for sim in simfolders:
            ...
            comb_writer.writerow([plot_fid,df_comb["i"].iloc[0],df_comb["c"].iloc[0],df_comb["fc"].iloc[0],df_comb["ks"].iloc[0], df_comb["por"].iloc[0], df_comb["r"].iloc[0],df_comb["rainfall"].iloc[0],
                                ...
                                df_comb["median"].iloc[0],df_comb["q2"].iloc[0],df_comb["p90"].iloc[0],df_comb["p95"].iloc[0],df_comb["cells"].iloc[0],
                                df_comb["cells_buffered"].iloc[0],df_comb["ls_failed"].iloc[0],df_comb["ls_failed_all"].iloc[0]])  
...
if __name__ == "__main__":
    ...
    cfg = {
        "result_base_path": "/mnt/data/analysis_inputs", # path where the simulations will be written
        ...
        "combination_name": "combination.csv", # filename for colected output
        ...
        "tar_outputs": True # whether to tar the simulation outputs before deleting the simulation folders
    }
